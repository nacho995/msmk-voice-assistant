...
// Resplandores centrales suaves para dar volumen
function dibujarResplandores(animationTime, intensity) {
    const r1 = orbRadius * (1.2 + 0.1 * Math.sin(animationTime * 1.2));
    const r2 = orbRadius * (2.4 + 0.15 * Math.sin(animationTime * 0.8 + 1.2));
    const i = Math.max(0.2, intensity);
    
    dibujarResplandor(centerX, centerY, r2 * 5.2, [0, 160, 255], 0.06 * i);
    dibujarResplandor(centerX, centerY, r2 * 3.8, [0, 200, 255], 0.08 * i);
    dibujarResplandor(centerX, centerY, r1 * 2.4, [120, 230, 255], 0.1 * i);
}
